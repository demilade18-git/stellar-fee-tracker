...
/// Build metadata collected at compile time.
#[derive(Debug, Clone)]
...
impl Default for BuildInfo {
    fn default() -> Self {
        Self {
            ...
            commit_sha: std::env::var("DEVKIT_COMMIT_SHA").unwrap_or_else(|_| "unknown".into()),
            build_time: std::env::var("DEVKIT_BUILD_TIME")
                .unwrap_or_else(|_| {
                    SystemTime::now()
            ...
            rustc_version: std::env::var("DEVKIT_RUSTC_VERSION")
                .unwrap_or_else(|_| "unknown".into()),
            target: std::env::var("DEVKIT_TARGET").unwrap_or_else(|_| "unknown".into()),
            debug: std::env::var("DEVKIT_DEBUG")
                .map(|v| v == "true")
                .unwrap_or(true),
            features: Vec::new(),
        }
...
impl BuildInfo {
    ...
    /// Compare two versions using semantic versioning logic.
...
pub struct VersionArgs {
    /// Output as JSON.
    pub json: bool,
    /// Check if the current version is newer than the given version.
